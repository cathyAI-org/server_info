...
**Storage Note:** HBA is passed through to the TrueNAS VM via PCIe passthrough.
...
**Role:** NPU-SVC, NPU-accelerated AI, Ollama API, AI Orchestrator, Prompt Composer  
**Planned:** Media LXC (Jellyfin + Arr-style media automation) alongside the AI LXC. Both are intended to share access to a future external GPU.  
...
**Role:** General compute, Chainlit WebUI  
**Candidate future role:** Disc ripping machine (ARM-style automated workflow) or other dedicated media-ingest role. Placement not finalized; may use a different dedicated mini PC instead.  
...
| **Connected Devices**  | Synology NAS, Philips TV, Zyxel switch, TP-Link switch |
...
- **IoT Devices:** Synology NAS, Eufy HomeBase 3 + 3 cameras, Philips TV
